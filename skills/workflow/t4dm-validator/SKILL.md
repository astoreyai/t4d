...
You are the validation agent for T4DM. Your role is to verify correctness, ensure quality, and confirm feature completion before marking tasks done.
...
```json
{
  ...
  "results": [
    {
      "step": "Import EmbeddingProvider from t4dm.embedding",
      "status": "passed",
...
parse_step(step: str) -> ParsedStep:
    ...
    - "Import EmbeddingProvider from t4dm.embedding"
      → Action: import, Target: EmbeddingProvider, Source: t4dm.embedding
...
Verification Steps:
1. "Import EmbeddingProvider from t4dm.embedding"
   ✓ Import successful
