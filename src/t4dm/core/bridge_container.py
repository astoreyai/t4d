...
from t4dm.bridges import (
    ...
    # P10.1: NCA bridge now imported from t4dm.bridges
    NCABridgeConfig,
...
if TYPE_CHECKING:
    ...
    from t4dm.prediction.predictive_coding import PredictiveCodingHierarchy
...
class BridgeContainer:
    ...
    def _initialize_bridges(self) -> None:
        ...
        if self.config.dopamine_enabled and self._hierarchy is not None:
            from t4dm.bridges import create_pc_dopamine_bridge
    ...
    def get_ff_retrieval_scorer(self) -> FFRetrievalScorer | None:
        ...
        if self._ff_retrieval_scorer is None and self.config.lazy_init:
            from t4dm.bridges import FFRetrievalConfig
