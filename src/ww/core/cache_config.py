...
class RedisCacheConfig:
    ...
    def from_env(cls) -> "RedisCacheConfig":
        ...
        Environment variables:
            WW_REDIS_URL: Redis connection URL
            WW_REDIS_ENABLED: Enable Redis cache (true/false)
            WW_CACHE_FALLBACK_ENABLED: Enable fallback cache (true/false)
            WW_CACHE_EMBEDDING_TTL: Embedding cache TTL in seconds
            WW_CACHE_SEARCH_TTL: Search cache TTL in seconds
            WW_CACHE_GRAPH_TTL: Graph cache TTL in seconds
        """
        import os

        redis_url = os.getenv("WW_REDIS_URL", "redis://localhost:6379")
        enabled = os.getenv("WW_REDIS_ENABLED", "true").lower() == "true"
        fallback_enabled = os.getenv("WW_CACHE_FALLBACK_ENABLED", "true").lower() == "true"

        # Tier TTLs
        embedding_ttl = int(os.getenv("WW_CACHE_EMBEDDING_TTL", "3600"))
        search_ttl = int(os.getenv("WW_CACHE_SEARCH_TTL", "300"))
        graph_ttl = int(os.getenv("WW_CACHE_GRAPH_TTL", "600"))

        # Tier max sizes
        embedding_max_size = int(os.getenv("WW_CACHE_EMBEDDING_MAX_SIZE", "5000"))
        search_max_size = int(os.getenv("WW_CACHE_SEARCH_MAX_SIZE", "1000"))
        graph_max_size = int(os.getenv("WW_CACHE_GRAPH_MAX_SIZE", "500"))
