...
def validate_password_strength(password: str, field_name: str = "password") -> str:
    ...
    if not password:
        raise ValueError(
            ...
            f"Set WW_{field_name.upper()} environment variable."
        )
...
def _find_config_file() -> Path | None:
    ...
    Search order:
    1. WW_CONFIG_FILE environment variable
    ...
    """
    # Check WW_CONFIG_FILE environment variable
    env_config = os.getenv("WW_CONFIG_FILE")
    if env_config:
        path = Path(env_config).expanduser()
        if path.exists():
            return path
        logger.warning(f"Config file from WW_CONFIG_FILE not found: {path}")
...
def _merge_config_with_env(yaml_config: dict) -> dict:
    ...
    Keys are converted from snake_case to WW_UPPER_CASE format.
...
def get_settings() -> Settings:
    ...
    - WW_CONFIG_FILE environment variable
