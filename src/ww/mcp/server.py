...
Usage:
    ...
    # Or programmatically
    from ww.mcp import run_mcp_server
    asyncio.run(run_mcp_server())

Configuration via environment:
    WW_API_URL: World Weaver API URL (default: http://localhost:8765)
    WW_SESSION_ID: Default session ID
    WW_API_KEY: Optional API key
"""
...
class WorldWeaverMCPServer:
    ...
    async def handle_list_resources(self) -> dict:
        ...
        resources = [
            {
                "uri": f"ww://sessions/{self.session_id}",
...
def create_mcp_server(
    ...
    Args:
        api_url: World Weaver API URL (default from WW_API_URL env)
        session_id: Session ID (default from WW_SESSION_ID env)
        api_key: API key (default from WW_API_KEY env)

    Returns:
        Configured MCP server
    """
    return WorldWeaverMCPServer(
        api_url=api_url or os.environ.get("WW_API_URL", "http://localhost:8765"),
        session_id=session_id or os.environ.get("WW_SESSION_ID"),
        api_key=api_key or os.environ.get("WW_API_KEY"),
    )
